...
class VOC12ImageDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
        ...
        self.sal_root = os.path.join(voc12_root,"Sal")
    ...
    def __getitem__(self, idx):
        ...
        sal = np.asarray(imageio.imread(os.path.join(self.sal_root,name_str+".png")))
        sal = sal/np.max(sal)
        if self.resize_long:
        ...
        return {'name': name_str, 'img': img, 'sal' : sal}
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root,
        ...
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.voc12_root = voc12_root
        self.sal_root = os.path.join(self.voc12_root, 'Sal')
    ...
    def __getitem__(self, idx):
        ...
        sal = imageio.imread(os.path.join(self.sal_root, name_str + '.png'))
        img, label = np.asarray(img), np.asarray(label)
        sal = np.asarray(sal)
        if self.rescale:
            img, label, sal = imutils.random_scale((img, label,sal), scale_range=self.rescale, order=(3, 0, 0))

        if self.img_normal:
            img = self.img_normal(img)
            sal = sal/np.max(sal)
        if self.hor_flip:
            img, label, sal = imutils.random_lr_flip((img, label, sal))

        if self.crop_method == "random":
            img, label, sal = imutils.random_crop((img, label, sal), self.crop_size, (0, 255, 0))
        else:
            img = imutils.top_left_crop(img, self.crop_size, 0)
            label = imutils.top_left_crop(label, self.crop_size, 255)
            sal = imutils.top_left_crop(sal, self.crop_size, 0)
        ...
        return {'name': name, 'img': img, 'label': label, 'sal': sal}
